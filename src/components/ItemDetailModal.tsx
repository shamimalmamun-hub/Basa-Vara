...
export default function ItemDetailModal({ property, tutor, onClose }: ItemDetailModalProps) {
  ...
  const shareUrl = typeof window !== 'undefined' 
    ? (property ? `${window.location.origin}/rentals?id=${property.id}` : `${window.location.origin}/tutors?id=${tutor?.id}`)
    : '';

  const shareTitle = property 
    ? property.title 
    : `${tutor?.name || 'টিউটর'} - বাসা ভাড়া ও হোম টিউটর`;
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: shareTitle,
          text: property ? `${property.title} - বাসা ভাড়া ও হোম টিউটর` : `${tutor?.name} (হোম টিউটর)`,
          url: shareUrl
        });
      } catch (err) {
      ...
      >
        ...
        >
          ...
          <div className="p-4 overflow-y-auto flex-1 min-h-0 space-y-3.5 text-slate-800 dark:text-slate-200 overscroll-contain">
            ...
            <div className="p-3 rounded-xl bg-slate-100/70 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center gap-1">
                  ...
                  <span>{language === 'bn' ? 'সোশ্যাল মিডিয়ায় শেয়ার করুন' : 'Share on Social Media'}</span>
                </span>
                {copied && (
                  <span className="text-[10px] font-extrabold text-emerald-600 dark:text-emerald-400 flex items-center gap-1 animate-fade-in">
                    <Check className="w-3 h-3" /> {language === 'bn' ? 'লিংক কপি হয়েছে!' : 'Link Copied!'}
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center justify-center gap-1.5 p-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg text-xs font-bold transition-all shadow-sm"
                  title="WhatsApp"
                >
                  <MessageCircle className="w-3.5 h-3.5" />
                  <span>WhatsApp</span>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center justify-center gap-1.5 p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-all shadow-sm"
                  title="Facebook"
                >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 p-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-200 rounded-lg text-xs font-bold transition-all shadow-sm cursor-pointer"
